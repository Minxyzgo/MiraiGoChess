...
class ChessPad(
    ...
    private var lastCoordinateX: Int = 0
    private var lastCoordinateY: Int = 0
    ...
    fun Stones.debug() {
        forEachIndexed { x, stone ->
            stone.forEachIndexed { y, color ->
                if(color != StoneColor.None) KotlinMain.logger.debug("color: $color x: $x, y: $y")
            }
        }
    }
    ...
    fun regretChess(): Int {
        ...
        val isBlackMoving = player.color == StoneColor.Black
        val lastChess = if(isBlackMoving) lastBlackChess else lastWhiteChess
        // 当玩家用完悔棋机会时
        if(player.regretChance <= 0) return 3

        return if(lastChess != null) {
            if(move === lastChess) {
                //当已经悔棋时， 应不能再继续悔棋， 抛出
                4
            } else {
        ...
        for (i in 0..18) {
            for (j in 0..18) {
                if (move[i][j] == StoneColor.Black) {
                    ...
                    Utils.drawTeNum(
                        (i + 1) * 25 + 10, (j + 1) * 25 + 10,
                        moveTeNum[i][j], this.graphics.create()
                    )
                }
